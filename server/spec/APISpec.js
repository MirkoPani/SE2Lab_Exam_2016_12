...
describe("Test /deleteStudent",function() {
    ...
    //legal request only using ID
	it("to returns status code 200", function(done) {
	  client.post(base_url + "deleteStudent/", dataID, function(err, res, body) {
		expect(body).toEqual(
			{
				ID: "1",
				SSN: "AB45", 
	...
	dataw = {ID: "10" };
	it("to returns status code 406", function(done) {
	  client.post(base_url + "searchStudent/", dataw, function(err, res, body) {
		expect(res.statusCode).toBe(404);
		done();
	  });
	});
    ...
    //set data only ssn
    var dataSSN= {SSN:'A6T4'};
    
    //legal request only using SSN
	it("to returns status code 200", function(done) {
	  client.post(base_url + "deleteStudent/", dataSSN, function(err, res, body) {
		expect(body).toEqual(
			{
				ID: "2",
				SSN: "A6T4", 
    ...
    //wrong parameter and missing ID and SSN
	data2 = {name: "1" };
	it("to returns status code 406", function(done) {
